def load_query (multiple_predict, project_name, reg_datekey_str):
    ...
    order by reg_datekey, datekey'''
        ...
        reg_datekey_list = "', '".join(reg_datekey_list)
    ...
    with sample as(
      ...
      from(
        ...
        from {project_name.lower()}_dw.f_user_map
        where login_flag = 1
        and datekey>= '{min_reg_datekey}' and datekey<= date_add('{max_reg_datekey}', interval 30 day)
    ...
    , sample2 as (
      select reg_datekey, datekey, reg_uu, reg_diff
      , DAU, retention, ARPU
      from sample
    )
    ...
    INNER JOIN (
      select reg_datekey,datekey, sum(daily_revenue) as daily_revenue
      from {project_name.lower()}_dw.f_user_map
      where datekey>= '{min_reg_datekey}' and datekey<= date_add('{max_reg_datekey}', interval 30 day)
        and reg_datekey in ('{reg_datekey_list}')
      group by reg_datekey,datekey
    )as B
    ON A.datekey = B.datekey and A.reg_datekey = B.reg_datekey
