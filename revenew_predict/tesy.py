...
for col in colnames:
    window_days = 7
    idx = df_s.reg_datekey == col
    ARPU_hat[col] = df_s.loc[idx, 'ARPU'].rolling(window = window_days, min_periods= window_days).mean()
    ARPU_hat.iloc[:window_days] = ARPU_hat.iloc[:window_days].fillna(0)

    # 이동평균 이후의 NA는 마지막값으로 채운다.
    idx = ARPU_hat.loc[:,col].notnull()
    last_ma = ARPU_hat.loc[idx].iloc[-1].values

    ARPU_hat.loc[~idx] = ARPU_hat.loc[~idx].fillna(last_ma[0])
